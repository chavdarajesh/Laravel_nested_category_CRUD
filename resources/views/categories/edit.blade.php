...
@section('context')
    ...
    <form id="categories_form" method="post" action="{{ route('categories.update',$category->id) }}">
        @csrf
        <div class="mb-3">
            <label for="category_name" class="form-label">Category Name</label>
            <input value="{{$category->category_name ? $category->category_name : old('category_name') }}" name="category_name" type="test" class="form-control" id="category_name" aria-describedby="category_name">
        ...
        <div class="mb-3">
            ...
            <select {{ count($categories) == 0 ? 'disabled' : '' }} name="parent_category_id" class="form-select"
                ...
                @foreach ($categories as $categorie)
                    <option {{$category->parent_category_id == $categorie->id ? 'selected' : '' }} value="{{ $categorie->id }}">{{ $categorie->category_name }}</option>
                @endforeach
            </select>
